...
class State():
	def __init__(self, w_pos, w_vel, w_accel, w_yaw, yaw_dot, dim=3):
		from .utils import posAccelYaw2TfMatrix
		assert w_pos.shape==(dim,1)
		assert w_vel.shape==(dim,1)
		assert w_accel.shape==(dim,1)
		...
		self.w_T_f= posAccelYaw2TfMatrix(self.w_pos, np.array([[0.0],[0.0], [0.0]]), w_yaw, dim=dim) #pos, accel, yaw
	...
	def f_vel(self):
		w_vel = self.w_vel
		if self.dim == 2:
			w_vel = np.vstack((w_vel, np.array([0.0])))

		f_vel=self.f_T_w.rot()@w_vel;
		...
		return f_vel[:self.dim];
	def f_accel(self):
		self.f_T_w.debug();
		w_accel = self.w_accel
		if self.dim == 2:
			w_accel = np.vstack((w_accel, np.array([0.0])))
		f_accel=self.f_T_w.rot()@w_accel;

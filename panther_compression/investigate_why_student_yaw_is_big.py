...
if __name__ == "__main__":
	...
	data = load(path_panther[:-8]+'/panther_compression/evals/tmp_dagger/2/investigate_why_student_yaw_is_big/dagger-demo-20230223_124952_f7e5c3.npz')
	lst = data.files
	...
	path_file=path_panther[:-8]+"/panther_compression/evals/tmp_dagger/2/investigate_why_student_yaw_is_big/intermediate_policy_round27_log9.pt"
	...
	acts_student = student_policy.forward(obs, deterministic=True)
	acts_student = acts_student.detach()
	# acts_n = student_policy.forward(obs, deterministic=True)
	# acts_n_student=(acts_n[0].reshape(venv.am.getActionShape()).detach().numpy()) 
	...
	num_of_traj_per_action=list(acts_expert.shape)[1] #acts_expert.shape is [batch size, num_traj_action, size_traj]
	num_of_elements_per_traj=list(acts_expert.shape)[2] #acts_expert.shape is [batch size, num_traj_action, size_traj]
	batch_size=list(acts_expert.shape)[0]

	# not sure why but list(acts_expert.shape)[0] is different from list(acts_student.shape)[0]
	...
	for i in range(num_of_traj_per_action):
		for j in range(num_of_traj_per_action):
			expert_i = acts_expert[:,i,:].float() #All the elements
			student_j = acts_student[:,j,:].float() #All the elements

			expert_pos_i = acts_expert[:,i,0:venv.am.traj_size_pos_ctrl_pts].float()
			student_pos_j = acts_student[:,j,0:venv.am.traj_size_pos_ctrl_pts].float()

			expert_yaw_i = acts_expert[:,i,venv.am.traj_size_pos_ctrl_pts:(venv.am.traj_size_pos_ctrl_pts+venv.am.traj_size_yaw_ctrl_pts)].float()
			...
			expert_time_i = acts_expert[:,i,-1:].float(); #Time. Note: Is you use only -1 (instead of -1:), then distance_time_matrix will have required_grad to false
			...
			expert_pos_j = acts_expert[:,j,0:venv.am.traj_size_pos_ctrl_pts].float();

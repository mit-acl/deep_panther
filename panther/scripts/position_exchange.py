...
class TermGoalSender:
    ...
    def __init__(self):
        ...
        # set time_init how long the demo is
        self.total_secs = 60.0; # sec
        self.timerCB_initialized = False
    ...
    def timerCB(self, tmp):
        ...
        if not self.timerCB_initialized:
            self.time_init = rospy.get_rostime().to_sec()
            self.timerCB_initialized = True

        # check if we should go home        
        duration = rospy.get_rostime().to_sec() - self.time_init
        print(f"{duration} seconds elapsed")
        if (duration > self.total_secs and not self.is_home):
            self.is_home = True
            self.sendGoal()
            self.timer.shutdown()
    ...
    def sendGoal(self):
        ...
        if self.is_home:
            ...
            # panther simulations
            if self.mode == 1:
                if not self.if_arrived:
                    self.term_goal.pose.position.x = 6.0
                    self.term_goal.pose.position.y = -2.5
                else:
                    self.term_goal.pose.position.x = 2.5
                    self.term_goal.pose.position.y = 2.0
            elif self.mode == 2:
                if self.if_arrived:
